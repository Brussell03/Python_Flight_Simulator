...
def forward_euler(f, t, x, h_s, vmod, cmod, x_trim_ref, dx, auxillary_data):
    """
    Performs forward Euler integration to approximate the solution of a differential equation.
    """

    dx, auxillary_data = f(t, x, dx, auxillary_data, x_trim_ref, vmod, cmod)
    x_new = x + h_s * dx
    
    return x_new, auxillary_data

def AB2(f, t, x, t_prev, x_prev, h_s, vmod, cmod, x_trim_ref, dx, auxillary_data, i):
    """
    Performs the 2nd order Adams-Bashforth method to approximate the solution of a differential equation.
    """

    fim1, auxillary_data = f(t, x, dx, auxillary_data, x_trim_ref, vmod, cmod)
    if i == 0:
        x_new = x + h_s * fim1
    else:
        fim2, _ = f(t_prev, x_prev, dx, auxillary_data, x_trim_ref, vmod, cmod)
        x_new = x + 1.5*h_s*fim1 - 0.5*h_s*fim2
    
    return x_new, auxillary_data


def RK4(f, t, x, h_s, vmod, cmod, x_trim_ref, dx, auxillary_data):
    """
    Performs the 4th order Runge-Kutta method to approximate the solution of a differential equation.
    """
    fim1_k1, auxillary_data = f(t, x, dx, auxillary_data, x_trim_ref, vmod, cmod)
    k1 = h_s*fim1_k1
    fim1_k2, _ = f(t + 0.5*h_s, x + 0.5*k1, dx, auxillary_data, x_trim_ref, vmod, cmod)
    k2 = h_s*fim1_k2
    fim1_k3, _ = f(t + 0.5*h_s, x + 0.5*k2, dx, auxillary_data, x_trim_ref, vmod, cmod)
    k3 = h_s*fim1_k3
    fim1_k4, _ = f(t + h_s, x + k3, dx, auxillary_data, x_trim_ref, vmod, cmod)
    k4 = h_s*fim1_k4 
    x_new = x + 1/6*(k1 + 2.0*k2 + 2.0*k3 + k4)

    return x_new, auxillary_data

def adaptive_integration(eom_func, t_span, t_eval, x0, vehicle, cmod, x_trim_ref, method='RK45', rtol=1e-6, atol=1e-6):
    """
    ...
    def eom_wrapper(t, x):
        ...
        eom_func(t, x, dx_tmp, aux_tmp, x_trim_ref, vehicle, cmod)
        return dx_tmp
    ...
    for i in range(nt):
        eom_func(t_out[i], x_out[:, i], dx_tmp, aux_tmp, x_trim_ref, vehicle, cmod)
