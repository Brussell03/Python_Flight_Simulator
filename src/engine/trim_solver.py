...
def trim_solver(eom, vehicle, amod, cmod, tmod, x):
    ...
    x_guess[13] = tmod.get('dela_ach_deg', x[14])
    x_guess[14] = tmod.get('dele_ach_deg', x[15])
    x_guess[15] = tmod.get('delr_ach_deg', x[16])
    x_guess[16] = tmod.get('delt_ach_pct', x[17])
    ...
    print(f"dela_deg:  {x_guess[13]:.8f}")
    print(f"dele_deg:  {x_guess[14]:.8f}")
    print(f"delr_deg:  {x_guess[15]:.8f}")
    ...
    x_e_m, y_e_m, z_e_m = eom.earth_model.wgs84_to_cartesian(lat_rad, long_rad, x_trim[11])
    ...
    dx, auxillary_data = eom.solve_eom(0, x_trim_full, dx, auxillary_data, None, vehicle, cmod)
    ...
    if result.success:
        ...
        print(f"dela_deg:  {x_trim[13]:.8f}")
        print(f"dele_deg:  {x_trim[14]:.8f}")
        print(f"delr_deg:  {x_trim[15]:.8f}")
        ...
        print(f"dela_ach_deg-dot: {dx[14]: .8f}")
        print(f"dele_ach_deg-dot: {dx[15]: .8f}")
        print(f"delr_ach_deg-dot: {dx[16]: .8f}")
        print(f"delt_ach_pct-dot: {dx[17]: .8f}")
        
        # Package the trim controls for external use (like the B Matrix numerical derivation)
        u_trim = np.array([
            x_trim_full[14], # dela_cmd
            x_trim_full[15], # dele_cmd
            x_trim_full[16], # delr_cmd
            x_trim_full[17]  # delt_cmd
        ])
