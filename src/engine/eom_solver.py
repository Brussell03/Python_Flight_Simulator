...
class eom_solver:
    def __init__(self, earth_model):
        self.earth = earth_model # Injected Earth Model
    ...
    def solve_eom(self, t, x, dx, auxillary_data, u_trim, vehicle, amod, cmod):
        ...
        lat_rad, long_rad, h_m = self.earth.ecef_to_geodetic(x_e_m, y_e_m, z_e_m)
        ...
        # Gravity & Earth Rates
        g_e_mps2 = self.earth.get_gravity_ecef(x_e_m, y_e_m, z_e_m)
        g_b_mps2 = C_e2b @ g_e_mps2
        omega_ie_b_rps = C_e2b @ self.earth.get_earth_rate_ecef()
        ...
        den_wgs84 = math.sqrt(1.0 - (self.earth.e * sin_lat)**2)
        RN_m = self.earth.a / den_wgs84
        RM_m = (self.earth.a * (1.0 - self.earth.e**2)) / (den_wgs84**3)
        
        omega_en_n_rps = np.array([v_n_mps[1] / (RN_m + h_m), -v_n_mps[0] / (RM_m + h_m), -v_n_mps[1] * math.tan(lat_rad) / (RN_m + h_m)])
        omega_ie_n_rps = np.array([self.earth.omega_rps * cos_lat, 0.0, -self.earth.omega_rps * sin_lat])
    ...
    def post_process(self, x, t_s, amod, auxillary_data):
        ...
        # for i in range(nt):
        #     lat_rad[i], long_rad[i], h_m[i] = self.earth.ecef_to_geodetic(x_e[i], y_e[i], z_e[i])
        vectorized_ecef_to_geodetic = np.vectorize(self.earth.ecef_to_geodetic)
        ...
        rho_kgpm3  = np.array([fastInterp1(amod["alt_m"], amod["rho_kgpm3"], alt) for alt in h_m])
        ...
        for i in range(nt):
            g_mag_mps2[i] = np.linalg.norm(self.earth.get_gravity_ecef(x_e[i], y_e[i], z_e[i]))

        # Reconstruct Body-to-NED rotations to extract Euler Angles and NED velocities
        phi_rad, theta_rad, psi_rad = np.zeros(nt), np.zeros(nt), np.zeros(nt)
        ...
        for i in range(nt):
            ...
            phi_rad[i]   = math.atan2(C_b2n[2, 1], C_b2n[2, 2])
            theta_rad[i] = math.asin(np.clip(-C_b2n[2, 0], -1.0, 1.0))
            psi_rad[i]   = math.atan2(C_b2n[1, 0], C_b2n[0, 0])
            
            v_b = np.array([u_b_mps[i], v_b_mps[i], w_b_mps[i]])
            v_n = C_b2n @ v_b
            u_n_mps[i], v_n_mps[i], w_n_mps[i] = v_n[0], v_n[1], v_n[2]
        ...
        return SimData(
            ...
            cs_mps=cs_mps, rho_kgpm3=rho_kgpm3, p_kgpms2=p_kgpms2, T_K=T_K,
